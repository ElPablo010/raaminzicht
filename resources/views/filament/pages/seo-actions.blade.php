<x-filament-panels::page>
    ...
    @forelse ($items as $item)
        @php
            ...
            $text = collect($sections)->firstWhere('section_type', 'prose');
            $sectionLabels = ['hero' => 'Hero + CTA', 'prose' => 'Tekst', 'faq' => 'FAQ', 'cta' => 'Afsluitende CTA'];
        ...
        <x-filament::section>
            ...
            @if ($editingId === $item['id'])
                ...
                <div style="display:flex;flex-direction:column;gap:.75rem;">
                    ...
                    @if ($item['action_type'] === 'create_page' && ! empty($editForm['sections']))
                        {{-- Pagina-opbouw: blokken verwijderen of verschuiven vóór publicatie --}}
                        <div style="font-size:.8rem;font-weight:600;">Pagina-opbouw</div>
                        <div style="display:flex;flex-direction:column;gap:.35rem;">
                            @foreach ($editForm['sections'] as $i => $s)
                                <div style="display:flex;align-items:center;gap:.5rem;border:1px solid rgba(128,128,128,.3);border-radius:.5rem;padding:.35rem .6rem;font-size:.85rem;">
                                    <span style="flex:1;">{{ $i + 1 }}. {{ $sectionLabels[$s['section_type'] ?? ''] ?? ($s['section_type'] ?? '?') }}</span>
                                    <button type="button" wire:click="moveSection({{ $i }}, -1)" @disabled($i === 0) title="Omhoog" style="cursor:pointer;background:none;border:none;color:inherit;opacity:{{ $i === 0 ? '.3' : '.7' }};">▲</button>
                                    <button type="button" wire:click="moveSection({{ $i }}, 1)" @disabled($i === count($editForm['sections']) - 1) title="Omlaag" style="cursor:pointer;background:none;border:none;color:inherit;opacity:{{ $i === count($editForm['sections']) - 1 ? '.3' : '.7' }};">▼</button>
                                    <button type="button" wire:click="removeSection({{ $i }})" title="Verwijderen" style="cursor:pointer;background:none;border:none;color:inherit;opacity:.7;">✕</button>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    ...
                    @if ($item['action_type'] === 'create_page')
                        <label style="font-size:.8rem;font-weight:600;">Titel van het tekstblok
                        ...
                        <label style="font-size:.8rem;font-weight:600;">Tekstblok (HTML toegestaan)
