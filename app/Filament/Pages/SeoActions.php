...
class SeoActions extends Page
{
    ...
    public function items(): array
    {
        return SeoActionItem::with('page:id,slug,title,is_homepage')
            ->latest()
            ->get()
            // Bewust NIET hersorteren op status: een net gepubliceerd item blijft
            // op zijn plek staan (met badge + "Bekijk pagina"), de filterknoppen
            // tonen het openstaande werk.
    ...
    public function startEdit(int $id): void
    {
        ...
        $text = collect($proposed['sections'] ?? [])->firstWhere('section_type', 'prose');
        ...
        $this->editForm = [
            ...
            // Kopie van de pagina-opbouw: de beheerder kan blokken verwijderen
            // of verschuiven vóór publicatie (enkel bij create_page).
            'sections' => array_values($proposed['sections'] ?? []),
        ];
        $this->editingId = $id;
    }
    ...
    /** Verschuif een blok in de pagina-opbouw van een create_page-voorstel. */
    public function moveSection(int $index, int $direction): void
    {
        $sections = array_values($this->editForm['sections'] ?? []);
        $target = $index + $direction;
        if (! isset($sections[$index], $sections[$target])) {
            return;
        }
        [$sections[$index], $sections[$target]] = [$sections[$target], $sections[$index]];
        $this->editForm['sections'] = $sections;
    }

    /**
     * Verwijder een blok uit de pagina-opbouw. Gaat de FAQ eruit, dan ook het
     * FAQ-bewerkveld — anders zet de submit-payload de sectie stiekem terug.
     */
    public function removeSection(int $index): void
    {
        ...
        $type = $sections[$index]['section_type'] ?? null;
        if ($type === 'faq') {
            $this->editForm['faq'] = [];
        }
        if ($type === 'prose') {
            $this->editForm['heading'] = '';
            $this->editForm['body'] = '';
        }
        unset($sections[$index]);
        $this->editForm['sections'] = array_values($sections);
    }
     ...
     * …) en patchen we enkel het "waarom"-tekstblok (`prose`) + de FAQ + meta.
    ...
    protected function buildProposed(SeoActionItem $item): array
    {
        ...
        if ($item->action_type === 'create_page') {
            // De (mogelijk herschikte of ingekorte) opbouw uit het bewerkscherm.
            $sections = array_values($f['sections'] ?? ($item->proposed['sections'] ?? []));

            // Patch het "waarom"-tekstblok (eerste prose) als het bewerkt is.
            if (trim((string) ($f['heading'] ?? '')) !== '' || trim((string) ($f['body'] ?? '')) !== '') {
                $patched = array_filter([
                    'heading' => $f['heading'] ?? null,
                    'body' => $f['body'] ?? null,
                ], fn ($v) => $v !== null && $v !== '');
                $idx = collect($sections)->search(fn ($s) => ($s['section_type'] ?? '') === 'prose');
                if ($idx !== false) {
                    $sections[$idx]['content'] = array_merge($sections[$idx]['content'] ?? [], $patched);
                } elseif ($patched) {
                    $sections[] = ['section_type' => 'prose', 'content' => $patched];
                }
            }
