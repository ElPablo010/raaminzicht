...
class SeoAdvisorService
{
    ...
    public function buildContext(): array
    {
        ...
        return [
            ...
            // First-party leads (zodra de attributie-kolommen gemigreerd zijn):
            // pagina's met bewezen conversie gaan voor op pagina's met enkel
            // vertoningen.
    ...
    /**
     * First-party gemeten conversies (aanvragen via de site) met herkomst.
     * Null zolang de attributie-kolommen ontbreken of er niets gemeten is.
     */
    protected function leadsContext(): ?array
    {
        if (! \Illuminate\Support\Facades\Schema::hasColumn('leads', 'channel')) {
            return null;
        }
...
PROMPT;
     ...
     * builder-contract van dit project (`prose` = heading+body, `faq` = items).
     *
     ...
     * generieke builder-blokken (hero → prose → faq → cta). De hero- en
    ...
    {
        $cta = $this->homepage()['cta'] ?? null; // ['label','href','page_id'] of null
        // In het CtaLinkSchema/PageLinkField-contract van dit project: een
        // interne pagina-link bewaart link_type=page + page_id (de href wordt
        // bij render en her-save uit page_id afgeleid), al de rest is een URL.
        // Zonder link_type zou een her-save in de admin de href wissen.
        $ctaButton = $cta ? [array_filter([
            'label' => $cta['label'],
            'variant' => 'primary',
            'link_type' => $cta['page_id'] ? 'page' : 'url',
            'page_id' => $cta['page_id'],
            'href' => $cta['href'],
        ], fn ($v) => $v !== null)] : null;
        ...
        if ($whyTitle !== '' || $whyBody !== '') {
            $sections[] = ['section_type' => 'prose', 'content' => array_filter([
                    ...
                    foreach ($c['ctas'] ?? [] as $cta) {
                        ...
                        if ($href = $this->resolveCtaHref($cta)) {
                            $anchors['cta'] ??= [
                                ...
                                'href' => $href,
                                'page_id' => $this->resolveCtaPageId($cta, $href),
                            ];
                            break;
                        }
                    }
    ...
    /**
     * De interne pagina achter een CTA, zodat de gekloonde knop als echte
     * pagina-link (link_type=page) bewaard kan worden. Null voor externe URL's.
     *
     * @param  array<string,mixed>  $cta
     */
    protected function resolveCtaPageId(array $cta, string $href): ?int
    {
        if (! empty($cta['page_id']) && Page::whereKey($cta['page_id'])->exists()) {
            return (int) $cta['page_id'];
        }
        ...
        return ($slug === ''
            ? Page::where('is_homepage', true)->first()
            : Page::where('slug', $slug)->first())?->id;
    }
